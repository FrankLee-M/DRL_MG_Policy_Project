...
from envs.surrogate.user_response_fitting_v0_0 import SurrogateUser
surrogate_user = SurrogateUser(save_dir="my_project/RA_obs/user_response_model")
...
#FIXME - 此处 ？obs_time_dim or obs_time_index ？
    ...
    def reset(self,
        ...
        if self.train_mode:
            # 训练模式：如果传入了seed，则重新初始化rng；否则使用defaul rng
            if seed is not None:
                self.env_rng = np.random.default_rng(seed)
                self.user_rng = np.random.default_rng(seed + 1)  # 确保用户模型的随机性与环境不同
            else:
            # 训练常用：若首次或尚未创建，则创建一次；之后沿用内部状态 / 初次需要初始化seed，后续传入None，进行演化
                if self.env_rng is None or self.user_rng is None:
                    self.env_rng  = np.random.default_rng()
                    self.user_rng = np.random.default_rng()    

##FIXME - ⚠️ 修改此处为，固定的 0 时刻为起点
            ...
            # #随机初始化 时间点
            # self.index_t = self.env_rng.integers(self.begin_t,self.end_t)
            # self._current_hour_index = calculate_time_indices(self.index_t)[0]  # hour_index             
       

        # test-mode 
        else:
            if seed is not None: #如果没有指定
                self.env_rng = np.random.default_rng(seed)
                self.user_rng = np.random.default_rng(seed+1)  #
#FIXME - test-seed 逻辑需要修改！
            else:
                self.env_rng = np.random.default_rng(self.test_seed)
                self.user_rng = np.random.default_rng(self.test_seed+1)  # 确保用户模型的随机性与环境不同
...
#NOTE - last_ave_sat_level 仅作为监测信息，把 last_complaint_cost 作为 obs 信息 
        # self.last_ave_sat_level = np.mean(np.asarray(self.User.last_sat_i)) 
        # 不重要的初始化  
        self.complaint_cost = 0.0
        ...
        if self.train_mode:
            self.surrogate_user.last_agg_load = self.last_agg_load
            # 如果能获取真实历史投诉最好，不能的话初始化为0或平均值
            self.surrogate_user.complaint_history = [0, 0, 0]
...
#NOTE - 记录历史数据，用于观测补全
        ...
        if self.train_mode:
            d_t,sat_t,complaint_number = self.surrogate_user.user_response_sample(lp,self._current_hour_index%24,lambda_hv) # type: ignore
            # 无意义
            d_i_t = np.zeros(self.User.Nd,)
            sat_i_t = np.zeros(self.User.Nd,)
        else:
...
if __name__ == "__main__":
    ...
    import argparse
    import numpy as np
    env = MgSurrogateEnv(is_user_dynamic=False,train_mode=True) 
