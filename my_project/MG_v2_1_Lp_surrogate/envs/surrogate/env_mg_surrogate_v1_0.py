...
from envs.surrogate.user_response_fitting_v1_0 import SurrogateUser
surrogate_user = SurrogateUser(save_dir="my_project/RA_obs/user_response_model_v1_0")
...
Major Changes:
- [BREAKING] modeling target changed from complaint count to cost

User Response Model:
    - user_response_fitting_v1_0 (Complaint-cost-based)
    - user_response_fitting_v1_3 (Complaint-cost-based)

Key Changes:
    - [ADD] unit_costs for complaint cost calculation (step & base-policy-step)
    - [DELETE] old complaint-number-based calcualtion
    - [DELETE] unnecessary info fields in step() 
"""
...
#NOTE - 记录历史数据，用于观测补全
        ...
        if self.train_mode:
            d_t,sat_t,complaint_cost = self.surrogate_user.user_response_sample(lp,self._current_hour_index%24,lambda_hv) # type: ignore
            # 无意义
