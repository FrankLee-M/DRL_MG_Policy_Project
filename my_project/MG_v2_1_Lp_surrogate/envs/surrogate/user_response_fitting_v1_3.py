...
### 拟合 投诉成本
# [修改] 定义保存目录
SAVE_DIR = "my_project/RA_obs/user_response_model_v1_3"
# [新增] 确保目录存在
...
class UserResponseCollector:
    ...
    def __init__(self, start_month=5, end_month=6):
        ...
        self.peak_hours = range(10, 22) 
        
       
        self.price_peak = 1.7  
        self.price_valley = 1.2
#ANCHOR - [修改] 电价决策逻辑：采用 峰谷分时电价策略        
    def collect_data(self):
        ...
        for t in range(self.start_idx, self.end_idx):
            ...
            if hour_index == 0:
                lmhv_day = lambda_hv_data[t:t+24] * self.env.range_lmhv
                lmhv_ave = lmhv_day.mean()
            if hour_index in self.peak_hours:
                lp_action = lmhv_ave * 1.7
            else:
                lp_action = lmhv_ave * 1.2
