...
""" 
User Response Model v0.0
Target:
    - Load (regression)
    - Complaint count (classifier + regressor)
Model:
    - RandomForestRegressor
    - RandomForestClassifier + RandomForestRegressor
Usage:
    Baseline complaint-based surrogate modeling.
"""


# [修改] 定义保存目录
SAVE_DIR = "my_project/RA_obs/user_response_model"
# [新增] 确保目录存在
...
class ResponseFitter:
    ...
    def fit_complaint_two_stage(self):
        ...
        # 且 dropna 已经处理过空值
        X = self.df[['hour', 'lambda_p','total_demand', 'lag_1', 'rolling_mean_3']]
...
class SurrogateUser:
    ...
    def user_response_sample(self, lambda_p, hour_index, lambda_hv):
        ...
        X_comp = pd.DataFrame([[hour_index, lambda_p, final_demand, lag_1, rolling_mean_3]],
                            columns=['hour', 'lambda_p' ,'total_demand', 'lag_1', 'rolling_mean_3'])
        
        # 执行预测
        if 'complaint_classifier' in self.models:
            has_complaint = self.models['complaint_classifier'].predict(X_comp)[0]
            if has_complaint == 0:
                pred_comp_mean = 0
            else:
...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        print(f"Step +{i+1}: Demand={d:.2f}, Prev Demand Internal={surrogate.last_agg_load:.2f}")