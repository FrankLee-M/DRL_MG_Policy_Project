import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import pickle
import os  # [新增] 用于处理路径
from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor, HistGradientBoostingRegressor
from scipy.stats import norm

# 引入你提供的环境模块
from envs.env_mg import MgComplaintFBEnv
from my_project.RA_obs.data_process import lambda_hv_data

"""
User Response Model v0.1

Target:
    - Same as v0 (complaint-based)

Key Difference:
    - Probabilistic inference during prediction
    - Addresses cold-start issue for complaint modeling

Backward Compatibility:
    - Compatible with v0 training data
"""



### 修改 v0 版本 complaint-预测 冷启动的问题
# [修改] 定义保存目录
SAVE_DIR = "my_project/RA_obs/user_response_model_v1"
# [新增] 确保目录存在
os.makedirs(SAVE_DIR, exist_ok=True)

class UserResponseCollector:
    """
    第一步：数据采集
    """
    def __init__(self, start_month=5, end_month=6):
        self.env = MgComplaintFBEnv(
            is_user_dynamic=False,
            train_mode=False
        )
        self.start_idx = 0
        self.end_idx = 8000
        
    def collect_data(self):
        print(f"Start collecting data from hour {self.start_idx} to {self.end_idx}...")
        
        data_list = []
        self.env.reset()
        
        # 重新初始化用户状态
        hour_idx_0 = self.start_idx % 24
        self.env.User._random_init_user_model(np.random.default_rng(100000), hour_idx_0)
        
        for t in range(self.start_idx, self.end_idx):
            hour_index = t % 24
            lmhv = lambda_hv_data[t] * self.env.range_lmhv
            
            lp_base = lmhv * 1.5
            lp_action = lp_base * np.random.uniform(0.9, 1.1) 
            lp_base = np.clip(lp_base, self.env.lp_min, self.env.lp_max)

            di, sat_i, comp_num = self.env.User.user_response([lp_action], hour_index)
            
            total_demand = np.sum(di)
            complaint_cost = comp_num / self.env.User.Nd
            
            record = {
                'time_idx': t,
                'hour': hour_index,
                'lambda_hv': lmhv,
                'lambda_p': lp_action,
                'price_ratio': lp_action / (lmhv + 1e-5),
                'total_demand': total_demand,
                'complaint_cost': complaint_cost,
                'complaint_count': comp_num,
            }
            data_list.append(record)
            
        df = pd.DataFrame(data_list)
        print(f"Data collection finished. Shape: {df.shape}")
        return df

class ResponseFitter:
    """
    第二步：分布拟合
    """
    def __init__(self, dataframe):
        self.df = dataframe.copy() # 使用副本
        self.models = {}
        self.residuals_dist = {}
        
    def visualize_data(self):
        """可视化数据分布"""
        plt.figure(figsize=(15, 5))
        
        plt.subplot(1, 3, 1)
        sns.histplot(self.df['total_demand'], kde=True)
        plt.title('Total Demand Distribution')
        
        plt.subplot(1, 3, 2)
        sns.scatterplot(x='lambda_p', y='total_demand', data=self.df, hue='hour', palette='viridis')
        plt.title('Price vs Demand')
        
        plt.subplot(1, 3, 3)
        sns.histplot(self.df['complaint_count'], bins=range(0, int(self.df['complaint_count'].max())+2))
        plt.title('Complaint Count Distribution')
        
        plt.tight_layout()
        # [修改] 正确的保存路径拼接
        plt.savefig(os.path.join(SAVE_DIR, "Data_distribution.png"))
        plt.close()

    def fit_demand_model(self):
        """拟合负荷模型"""
        print("Fitting Demand Model...")
        
        self.df['prev_demand'] = self.df['total_demand'].shift(1)
        self.df.dropna(inplace=True)
        
        X = self.df[['hour', 'lambda_p', 'prev_demand']]
        y = self.df['total_demand']
        
        rf = RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42)
        rf.fit(X, y)
        self.models['demand_regressor'] = rf
        
        y_pred = rf.predict(X)
        residuals = y - y_pred
        
        mu, std = norm.fit(residuals)
        self.residuals_dist['demand_std'] = std
        print(f"Demand Model fitted. Residual STD: {std:.4f}")
        
        # 可视化残差
        plt.figure()
        sns.histplot(residuals, kde=True)
        plt.title("Demand Residuals Distribution")
        # [修改] 正确路径
        plt.savefig(os.path.join(SAVE_DIR, "Demand_Residuals_Distribution.png"))
        plt.close()
        
        # 可视化散点图
        plt.figure(figsize=(8, 6))
        plt.scatter(y, y_pred, alpha=0.5)
        plt.title("Demand: Actual vs Predicted")
        # [修改] show 不接受保存参数，改为 savefig
        plt.savefig(os.path.join(SAVE_DIR, "Demand_Actual_vs_Predicted.png"))
        plt.close()

    def fit_complaint_two_stage(self):
        print("Fitting Complaint Two-Stage Model...")
        
        # 构造特征
        self.df['lag_1'] = self.df['complaint_count'].shift(1)
        self.df['rolling_mean_3'] = self.df['complaint_count'].rolling(window=3).mean()
        self.df.dropna(inplace=True)

        # 注意：这里的特征列表必须包含 prev_demand，因为前面的 fit_demand_model 已经生成了这一列
        # 且 dropna 已经处理过空值
        X = self.df[['hour', 'lambda_p', 'prev_demand','total_demand', 'lag_1', 'rolling_mean_3']]
        y = self.df['complaint_count']

        # --- 第一阶段：分类 ---
        y_binary = (y > 0).astype(int)
        rf_classifier = RandomForestClassifier(n_estimators=100, class_weight='balanced', random_state=42)
        rf_classifier.fit(X, y_binary)
        self.models['complaint_classifier'] = rf_classifier
        
        # --- 第二阶段：回归 ---
        mask_positive = y > 0
        X_pos = X[mask_positive]
        y_pos = y[mask_positive]
        
        if len(y_pos) > 0:
            rf_regressor = RandomForestRegressor(n_estimators=100, max_depth=5, random_state=42)
            rf_regressor.fit(X_pos, y_pos)
            self.models['complaint_regressor'] = rf_regressor
        else:
            print("警告：没有足够的正样本训练回归模型！")

        # 预测与残差
        prob_has_complaint = rf_classifier.predict(X) 
        predicted_amount = rf_regressor.predict(X)
        final_pred = prob_has_complaint * predicted_amount
        
        residuals = y - final_pred
        self.residuals_dist['complaint_std'] = np.std(residuals)
        print(f"Two-Stage Model fitted. Residual STD: {np.std(residuals):.4f}")
        
        # 绘图 1
        plt.figure(figsize=(8, 6))
        sns.histplot(residuals, kde=True)
        plt.title('Complaint Residuals Distribution')
        plt.xlabel('Residuals (Error)')
        plt.ylabel('Count')
        # [修改] 正确保存
        plt.savefig(os.path.join(SAVE_DIR, "Complaint_Residuals_Distribution.png"))
        plt.close()
        
        # 绘图 2
        plt.figure(figsize=(10, 6))
        plt.scatter(y, final_pred, alpha=0.5, color='blue', label='Data Points')
        max_val = max(y.max(), final_pred.max())
        plt.plot([0, max_val], [0, max_val], 'r--', lw=2, label='Perfect Prediction')
        plt.xlabel('Actual Complaint Count')
        plt.ylabel('Predicted Complaint Count')
        plt.title('Actual vs Predicted (Two-Stage Model)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        # [修改] 正确保存
        plt.savefig(os.path.join(SAVE_DIR, "Actual_vs_Predicted_Complaint.png"))
        plt.close()

    def save_models(self, name='user_surrogate_model.pkl'):
        save_dict = {
            'models': self.models,
            'residuals': self.residuals_dist
        }
        # [修改] 使用 os.path.join 确保路径正确
        filename = os.path.join(SAVE_DIR, name)
        with open(filename, 'wb') as f:
            pickle.dump(save_dict, f)
        print(f"Models saved to {filename}")

# --- 供外部调用的代理类 ---
class SurrogateUser:
    """
    第三步：代理模型类
    """
    def __init__(self, save_dir =SAVE_DIR, model_filename='user_surrogate_model.pkl'):
        # [修改] 默认路径加上 SAVE_DIR，否则加载会失败
        model_path = os.path.join(save_dir, model_filename)
        
        with open(model_path, 'rb') as f:
            data = pickle.load(f)
        self.models = data['models']
        self.residuals = data['residuals']
        self.Nd = 50 
        
            
    def user_response_sample(self, lambda_p, hour_index, lambda_hv):
        """
        替代 User.user_response()
        """
        # 初始化状态
        if not hasattr(self, 'complaint_history'):
            self.complaint_history = [0, 0, 0]
        if not hasattr(self, 'last_agg_load'):
            self.last_agg_load = 200.0
        
        # --- 1. 预测负荷 ---
        # 特征顺序: ['hour', 'lambda_p', 'prev_demand']
        X_demand = pd.DataFrame([[hour_index, lambda_p, self.last_agg_load]], 
                                columns=['hour', 'lambda_p', 'prev_demand'])
        
        pred_demand_mean = self.models['demand_regressor'].predict(X_demand)[0]
        
        # 采样噪声
        noise_demand = np.random.normal(0, self.residuals.get('demand_std', 1.0))
        final_demand = pred_demand_mean + noise_demand
        


        # --- 2. 预测投诉 ---            
        # A. 构造特征
        lag_1 = self.complaint_history[-1]
        rolling_mean_3 = np.mean(self.complaint_history[-3:])
        
        X_comp = pd.DataFrame([[hour_index, lambda_p, self.last_agg_load, final_demand, lag_1, rolling_mean_3]],
                            columns=['hour', 'lambda_p','prev_demand' ,'total_demand', 'lag_1', 'rolling_mean_3'])
        
        # B. 执行预测 (使用概率而不是硬分类)
        pred_comp_mean = 0.0
        
    
        # 获取分类器认为 "有投诉 (Class 1)" 的概率
        # predict_proba 返回 [prob_0, prob_1]
        probs = self.models['complaint_classifier'].predict_proba(X_comp)[0]
        prob_complaint = probs[1] 
        
        # 策略 1: 软阈值 (如果概率 > 20% 就认为可能投诉，而不是默认的 50%)
        # if prob_complaint > 0.2: 
        
        # 策略 2 (推荐): 伯努利采样 (Bernoulli Sampling)
        # 根据概率随机决定是否进入回归分支。这能完美模拟不确定性。
        is_complaint_active = np.random.rand() < prob_complaint
        
        if is_complaint_active:
            pred_comp_mean = self.models['complaint_regressor'].predict(X_comp)[0]
        else:
            pred_comp_mean = 0.0

        
        # C. 采样噪声并后处理
        # 即使预测均值很小，只要不为0，加上噪声后也有可能变成 1
        if pred_comp_mean <= 1e-5:
            final_comp = 0
        else:
            noise_comp = np.random.normal(0, self.residuals.get('complaint_std', 0.5))
            final_comp = pred_comp_mean + noise_comp
            final_comp = max(0, int(round(final_comp))) 
        

        # --- 3. 更新投诉状态 ---
        self.complaint_history.append(final_comp)
        if len(self.complaint_history) > 24:
            self.complaint_history.pop(0)
        self.last_agg_load = final_demand
        
        # 返回结果
        dummy_sat = np.zeros(getattr(self, 'Nd', 1)) 
        
        return final_demand, dummy_sat, final_comp
    
# ==========================================
# 主执行流程
# ==========================================

if __name__ == "__main__":
    # 1. 收集
    collector = UserResponseCollector()
    df_data = collector.collect_data()
    
    # 2. 拟合
    fitter = ResponseFitter(df_data)
    # fitter.visualize_data()
    fitter.fit_demand_model()
    fitter.fit_complaint_two_stage()
    fitter.save_models()
    
    # 3. 测试代理模型
    print("\nTesting Surrogate Model...")
    
    # 初始化环境
    env = MgComplaintFBEnv(is_user_dynamic=True, train_mode=True)
    env.reset()
    
    # 初始化代理 (此时会自动去 SAVE_DIR 找模型)
    surrogate = SurrogateUser()
    
    # 构造测试用例
    test_time_idx = 100 
    test_hour = test_time_idx % 24
    
    # 获取真实 lambda_hv 用于对比 (虽然模型输入现在不依赖它，但作为上下文打印出来也好)
    real_lmhv_val = lambda_hv_data[test_time_idx] * env.range_lmhv
    
    test_lp = 15.0 
    
    # A. 代理模型预测
    d_pred, _, c_pred = surrogate.user_response_sample(test_lp, test_hour, real_lmhv_val)
    
    print(f"Scenario: Time={test_time_idx}, Hour={test_hour}, Real HV Price={real_lmhv_val:.2f}, Test LP={test_lp}")
    print(f"[Surrogate] Demand={d_pred:.2f}, Complaints={c_pred}")
    
    # B. 验证逻辑
    print("--- Verifying against Data Collection Logic ---")
    
    # 手动同步环境状态
    env.User._random_init_user_model(np.random.default_rng(42), test_hour) 
    env.index_t = test_time_idx
    
    di_true, _, comp_true = env.User.user_response([test_lp], test_hour)
    d_true = np.sum(di_true)
    
    print(f"[True Env ] Demand={d_true:.2f}, Complaints={comp_true}")
    
    err_demand = abs(d_pred - d_true)
    print(f"Demand Error: {err_demand:.2f} ({(err_demand/d_true)*100:.2f}%)")
    
    # [新增] 连续多步测试，验证 last_agg_load 是否更新
    print("\n--- Running Multi-step Verification ---")
    for i in range(3):
        h = (test_hour + i + 1) % 24
        d, _, c = surrogate.user_response_sample(test_lp, h, real_lmhv_val)
        print(f"Step +{i+1}: Demand={d:.2f}, Prev Demand Internal={surrogate.last_agg_load:.2f},complaint-num:{c}")