...
'''
可用于 
    run_sac_training_env_surrogate_v0.py 中 env-version：“v0” 的测试
Strategy        | Avg Reward   | Avg Complaints -number
'''
...
TIMESTAMP = "160041"  # <--- 请修改为你实际运行生成的时间戳
...
def make_test_env(seed: int, param_path: str, cost_limit: float):
    ...
    env = make_base_env(env_version=env_version)
...
def evaluate_model(env, agent, test_episodes=100):
    ...
    # 存储结果，增加 Random
    results = {
        'RL': {'rewards': [], 'profits': [], 'complaints': [], 'prices': [], 'demands': []},
        'Baseline': {'rewards': [], 'profits': [], 'complaints': [], 'prices': [], 'demands': []},
        'Random': {'rewards': [], 'profits': [], 'complaints': [], 'prices': [], 'demands': []}
    }
    ...
    for episode in range(test_episodes):
        ...
        while not done:
            ...
            ep_complaints += info.get('complaint_count', 0)
        ...
        results['RL']['complaints'].append(ep_complaints)
        if episode == test_episodes - 1:
        ...
        while not done:
            ...
            ep_complaint_base += info.get('complaint_count', 0)
        ...
        results['Baseline']['complaints'].append(ep_complaint_base)
        if episode == test_episodes - 1:
        ...
        while not done:
            ...
            ep_complaint_rand += info.get('complaint_count', 0)
        ...
        results['Random']['complaints'].append(ep_complaint_rand)
        if episode == test_episodes - 1:
...
def plot_results(results,save_path):
    ...
    mean_rl_c = np.mean(results['RL']['complaints'])
    mean_base_c = np.mean(results['Baseline']['complaints'])
    mean_rand_c = np.mean(results['Random']['complaints'])
    ...
    for strategy, metrics in results.items():
        ...
        for i in range(num_episodes):
            summary_rows.append({
                ...
                'Complaints': metrics['complaints'][i],
                # 如果有 profit 也可以加进来
